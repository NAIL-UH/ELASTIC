...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        if opt.task in {'shrink', 'gather', 'subnet'}:
            ckpt = torch_load(opt.filename, map_location="cpu")["model"].float().state_dict()
            model.load_state_dict(ckpt, strict=False)
        if opt.task == 'subnet':
            subnet = {
                'ks': [3, 3, 5, 3, 5, 3, 5, 3, 5, 5, 5, 5, 5, 3, 5, 1, 5, 3, 3, 1, 1, 5, 5, 5, 1, 3, 5],
                'e': [5, 6, 2, 4, 6, 1, 3, 1, 3, 2, 5, 6, 1, 1, 4, 1, 1, 6, 1, 6, 1, 4, 6, 3, 6, 4, 2],
                'd': [5, 5, 5, 4, 1, 1, 1, 1, 1, 1, 1]
            }
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.task == 'gather':
            ...
            module.set_constraint([opt.k], 'kernel_size')
            module.set_constraint([opt.e], 'expand_ratio')
            module.set_constraint(module.depth_list)

            subnet = None
            if RANK in {-1, 0}:
                subnet = module.sample_active_subnet()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.task == 'shrink':
                ...
                module_depth_list = module.depth_list
                stages = len(module_depth_list) - 1

                depth_list = [module_depth_list[i:] for i in range(len(module_depth_list) - 2, -1, -1)]
                threshold = epochs // stages

                for stage in range(1, stages + 1):
                    if epoch < threshold*stage:
                        module.set_constraint(depth_list[stage - 1])
                        break
                ...
                if RANK in {-1, 0}:
                    subnet = module.sample_active_subnet()
                if RANK != -1:
                    broadcast_list = [subnet if RANK == 0 else None]
                    dist.broadcast_object_list(broadcast_list, 0)
                    if RANK != 0:
                        subnet = broadcast_list[0]
                        module.set_active_subnet(**subnet)
            ...
            if opt.task in {'train', 'shrink', 'subnet'}:
                # Backward
        ...
        if RANK in {-1, 0}:
            ...
            if opt.task == 'gather':
                data['subnet'].append(subnet)
                data['acc'].append(results)
                df = pd.DataFrame(data)
                df.to_excel(opt.outpath + f'/outputk{opt.k}e{opt.e}.xlsx', index=False)
...
def parse_opt(known=False):
    ...
    # ofa arguments
    parser.add_argument("--task", type=str, default="train", help="train / shrink / gather / subnet")
    parser.add_argument("--filename", type=str, default="", help="filename to desired weights")
    parser.add_argument("--outpath", type=str, default="", help="outpath to desired output")
    parser.add_argument("--k", type=int, default=5, help="Kernel size")
    parser.add_argument("--e", type=int, default=6, help="Expand ratio")
